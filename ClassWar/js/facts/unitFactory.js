  ...
  var UnitClass = function (argument) {
    ...
    it.unitId = argument.unitId;
  	...
  	it.carry = argument.carry; //how much does it carry resources for example
		it.alt = it.class + " player " + it.playerId;
		it.img = "img/units/unit.png"
  	it.res = {
  		id: null,
  		type: null,
  		quantity: 0
      ...
      eventStr = "";
  		if (what.isFree === true){
        eventStr = "unit" + it.unitId + ' moved from x:' + it.coords.x + ' y:' + it.coords.y + ' ';
  			it.move(what);
        eventStr += 'to x:' + it.coords.x + ' y:' + it.coords.y;
        console.log(eventStr);
  			return it;
  		} else
  		if (what.isResource === true){
  			it.gather(what);
        eventStr = "unit" + it.unitId + ' gathered ' + it.res.quantity +' ' + it.res.type;
  			console.log(eventStr);
      ...
      service.map.cells[it.locate()].isFree = true;
  		service.map.cells[it.locate()].isUnit = false;
    ...
    it.locate = function () { // turn pair coordinates (x,y) into 1 num, to use in Array
      ...
      eventStr += "player"  + it.playerId + " population:" + service.player[it.playerId].pop;
      console.log();
  		if (service.player[it.playerId].pop<=0) {
        eventStr += 'player' + it.playerId + ' lose';
  			console.log();
  		}
      ...
      eventStr = "unit" + it.unitId + ' hit unit' + this.dp + ' damage to unit' + it.targ.id;
  		console.log(eventStr);
  		if (unit[it.targ.id].hp <= 0){
        var oldResLenght = service.res.length;
        var targetIndex = service.map.locate(target.coords.x, target.coords.y, service.map.size.width);
        eventStr += "unit" + it.unitId + ' killed unit' + it.targ.id;
        console.log(eventStr);
